...
namespace NetworkUtil
{
  ...
  public class NetworkTests
  {
    ...
    private SocketState testLocalSocketState, testRemoteSocketState, newClient;
    ...
    public void TestReceiveHugeMessage(bool clientSide)
    {
      ...
      Assert.AreEqual(message.ToString(), testLocalSocketState.GetData());
        ...
        {
            NetworkTestHelper.SetupSingleConnectionTest(out testListener, out testLocalSocketState, out testRemoteSocketState);
            NetworkTestHelper.AddAnotherClient(out testListener, out newClient);

            Assert.IsTrue(newClient.TheSocket.Connected);

            Assert.AreEqual("127.0.0.1:2112", newClient.TheSocket.RemoteEndPoint.ToString());
        }

        [DataRow(true)]
        [DataRow(false)]
        [DataTestMethod]
        public void TestSendTinyMessageandClose(bool clientSide)
        {
            SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
        ...
        [DataRow(false)]
        [DataTestMethod]
        public void Test1(bool clientSide)
        {
            NetworkTestHelper.SetupSingleConnectionTest(out testListener, out testLocalSocketState, out testRemoteSocketState);
        ...
        public void TestConnectBadIP()
        {
            ...
            testListener = Networking.StartServer(saveServerState, 2112);
            Networking.ConnectToServer(saveClientState, "localhost", 1234);
